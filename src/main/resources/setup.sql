...
BEGIN
    RETURN QUERY SELECT m.id, m.match_date::TEXT, m.match_duration::TEXT, m.tournament::TEXT,
        m.game::TEXT, m.format::TEXT, m.team1::TEXT, m.team2::TEXT, COALESCE(m.winner,'')::TEXT
...
BEGIN
    RETURN QUERY SELECT m.id, m.match_date::TEXT, m.match_duration::TEXT, m.tournament::TEXT,
        m.game::TEXT, m.format::TEXT, m.team1::TEXT, m.team2::TEXT, COALESCE(m.winner,'')::TEXT
    FROM matches m WHERE m.tournament ILIKE '%' || p_query || '%'
...
BEGIN
    ...
    VALUES (p_date::DATE, trim(p_duration), trim(p_tourn), trim(p_game), trim(p_format),
            trim(p_team1), trim(p_team2),
...
BEGIN
    EXECUTE format('GRANT CONNECT ON DATABASE %I TO es_admin, es_guest', db);
END $$;
...
-- Гость — только разрешённые функции
